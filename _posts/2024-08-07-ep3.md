...
안타깝게도 제출한 논문에서는 설명력에 대한 정상적인 비교만을 제시할 뿐(제안한 기술은 명확한 소스코드를 가지고 있어 동작을 사용자가 쉽게 이해할 수 있다. vs 인공신경망은 동작이 복잡해 사용자가 쉽게 이해할 수 없다.), 정량적인 설명력 비교 실험 결과를 제시하지 못하여 아래와 같은 리뷰를 받았었다.
